function [F0,Fn,Zn,F] = EPG_GRE_TFE(theta,phi,TR,T1,T2,varargin)
%   [F0,Fn,Zn,F] = EPG_GRE_TFE(theta,phi,TR,T1,T2,varargin)
% Adapted from Shaihan Malik's EPG_GRE code.
%
...
if exist('diff','var')
    [ES, b] = set_relax_mat(T1, T2, TR, kmax, N, S, diff) ;
else
    [ES, b] = set_relax_mat(T1, T2, TR, kmax, N, S) ;
end
...
%     if jj==250
%         disp(['Resetting ES and b'])
%         [ES, b] = set_relax_mat(T1*0.5, T2, TR, kmax, N, S) ;
%     end
