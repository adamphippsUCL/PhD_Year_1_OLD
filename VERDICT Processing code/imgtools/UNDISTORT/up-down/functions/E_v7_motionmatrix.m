function [u] = E_v7_motionmatrix(v,arg)
...
y_coord = -floor(sy/2):ceil(sy/2)-1;
    ...
    %    v=v./(sqrt(sum(abs(S).^2,3)+eps)); %ori
...
%          v(isnan(v))=0;
        ...
        motion_matrix=arg.motion_matrix;
        v=reshape(motion_matrix*double(v(:)),size(v));
       % v = v .* S;  %usman
        for chn = 1:nch
            v_c=v.*S(:,:,chn);%usman
            %v_c = v(:,:,chn);%usman
            ...
            parfor ii = 1:num_time_points %par
                ...
                phase_pix = ( kx(ii) .* x_coord_mat ) + ( ky(ii) .* (y_coord_mat+offset) ) + ( t_s(ii) .* dB0_rads ); %               
