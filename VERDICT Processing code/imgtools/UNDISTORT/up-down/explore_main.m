...
clear all;
data_path = pref_uigetdir('UNDISTORT','data') ;

if ispc
    ext_dir = 'C:\Users\ucacdat\home\matlab\imgtools-external' ;
else
    ext_dir = '/Users/davidatkinson/matlab/external' ;
end

if ~exist(ext_dir,'dir')
    warning([ext_dir,' does not exist as a folder for external files.'])
end

UPDOWN_set_path( ext_dir ) ; % for external (downloaded) files
...
MR_DOWN=MRecon(fullfile(data_path, '20170630_115639_SE_DWI_0_500_PE_A_SENSEON_HalfScanOn_2mmRes_B0MapDE.lab'));
MR_UP=MRecon(fullfile(data_path, '20170630_115737_SE_DWI_0_500_PE_P_SENSEON_HalfScanOn_2mmRes_B0MapDE.lab'));
MR_dB0=MRecon(fullfile(data_path, '20170630_115334_B0map_2mmIsoRes_DE.lab')) ;
ref=MRecon(fullfile(data_path, '20170630_113159_SenseRefScan.lab')) ;
coilsurvey=MRecon(fullfile(data_path, '20170630_112927_CoilSurveyScan.lab') );

%%
S = MRsense( ref, MR_DOWN, coilsurvey );
S.Smooth=1;
S.Mask=1;
S.Extrapolate=1;

MR_DOWN.Parameter.Recon.SENSERegStrength=1e-3;
...
MR_DOWN.RingingFilter;
MR_DOWN.Parameter.Recon.EPICorrectionMethod='nonlin';
MR_DOWN.Parameter.Recon.EPI2DCorr='yes';
%MR_DOWN.ZeroFill;
MR_DOWN.K2IM;
...
disp(['Done initial MR_DOWN'])

%%


S = MRsense( ref, MR_UP, coilsurvey );S.Smooth=1;
% Error - should not be repeated?  S = MRsense( ref, MR_UP, coilsurvey );S.Smooth=1;
S.Mask=1;
S.Extrapolate=1;

 MR_UP.Parameter.Recon.SENSERegStrength=1e-3;
...
MR_scan=MR_UP;
MR_dB0=MR_dB0;
A = MR_scan.Transform('ijk', 'RAF',1 ); % 4x4 affine matrix
...
niftypath = '/Users/davidatkinson/niftyreg/niftyreg_install/bin' ;
data_path = '/Users/davidatkinson/nifty_data' ;
...
save_nii(imnii1,fullfile(data_path,'epi1.nii'));

clear imnii1;imnii1 = make_nii(abs(im22));
save_nii(imnii1,fullfile(data_path,'epi2.nii'));

% DA niftypath='D:\Demo_code\NiftyReg\';

% DA data_path='D:\Demo_code\NIFTI\nifty_data\';

% DA removed trailing spaces in epi and t2w
epi='epi1.nii';
t2w='epi2.nii';
