% DA explore_main_demo_joint
% From Usman's main_demo_joint
...
data_path = pref_uigetdir('UNDISTORT','joint') ;

ext_dir = '/Users/davidatkinson/matlab/external' ;
UPDOWN_set_path(ext_dir)


MR_DOWN=MRecon(fullfile(data_path, 'i_19012018_1129396_9_2_wip_se_dwi_0_500_1000_pea_sense2_halfscanon_te55ms_senseV4.lab'));
MR_UP=MRecon(fullfile(data_path, 'i_19012018_1130296_10_2_wip_se_dwi_0_500_1000_pep_sense2_halfscanon_te55ms_senseV4.lab'));
MR_dB0=MRecon(fullfile(data_path, 'i_19012018_1123216_5_2_wip_b0map_de_3d_nosense_pe_rl_fatshifta_clearV4.lab'));
ref=MRecon(fullfile(data_path, 'i_19012018_1119497_1000_11_wip_senserefscan_clearV4.lab'));
coilsurvey=MRecon(fullfile(data_path, 'i_19012018_1119266_1000_8_wip_coilsurveyscanV4.lab'));
...
MR_DOWN.Parameter.Recon.EPICorrectionMethod='nonlin';MR_DOWN.Parameter.Recon.EPI2DCorr='yes';
%MR_DOWN.ZeroFill;
MR_DOWN.K2IM;
...
S = MRsense( ref, MR_UP, coilsurvey );
S.Smooth=1;
S.Mask=1;
S.Extrapolate=1;
...
avg=1;dir=1;
Sk11=double(squeeze(  kspace_up(:,:,1,1,1,1,1,islice,1,b_value,dir,avg)));
...
b_value=2;
for avg=1:size(kspace_up,12);
    for dir=1:size(kspace_up,11);
        Sk11=double(squeeze(kspace_up(:,:,1,1,1,1,1,islice,1,b_value,dir,avg)));
        Sk22=double(squeeze(kspace_down(:,:,1,1,1,1,1,islice,1,b_value,dir,avg)));
        y=[];
        temp1=Sk11;
        ...
        Gb1 = block_fatrix(Gc1, 'type', 'col'); 
        xr = qpwls_pcg1(0*zeros(row,col), Gb1, f.wi, Sk11(:), R1.C, 'niter', 30);
        xunc1 = ig.embed(xr);
        ...
        xunc = ig.embed(xr);rec_corr(:,:,dir,avg)=xunc;
    end
end
...
for avg=1:size(kspace_up,12)
    for dir=1:size(kspace_up,11)
        ...
        Gb2 = block_fatrix(Gc2, 'type', 'col'); % [G1; G2; ... ]
        xr = qpwls_pcg1(0*zeros(row,col), Gb2, f.wi, y, R1.C, 'niter', 30);
        xunc = ig.embed(xr);rec_nocorr(:,:,dir,avg)=xunc;
    end
end
...
for avg=1:size(kspace_up,12);
    for dir=1:size(kspace_up,11);
        ...
        Gb1 = block_fatrix(Gc1, 'type', 'col'); 
        xr = qpwls_pcg1(0*zeros(row,col), Gb1, f.wi, Sk22(:), R1.C, 'niter', 30);
        xunc2 = ig.embed(xr);
        ...
        Gb2 = block_fatrix(Gc2, 'type', 'col'); % [G1; G2; ... ]
        xr = qpwls_pcg1(0*zeros(row,col), Gb2, f.wi, y, R1.C, 'niter', 30);
        xunc = ig.embed(xr);rec_init(:,:,dir,avg)=xunc;
    end
end
