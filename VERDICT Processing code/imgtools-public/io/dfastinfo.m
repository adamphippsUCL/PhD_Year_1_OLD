...
load(dict,'tags','values') % loads tags and values
...
if fid < 1
    warning(message) 
    disp(['File attempted to open was ',dcmfile])
    fclose(fid)
    dinfo = [];
    return
end
...
while(ftell(fid) < 132+g2len+1) % +1 added 2 Nov 2022 possibly length in file is too short if next to last value is padded?
