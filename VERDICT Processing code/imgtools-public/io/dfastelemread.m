...
switch plicity
    ...
    case 'implicit'
        ...
        if nnz(idx)==0   % idx is sparse
            if rem(group,2) == 0
                % Only issue warning for non-private tags 
                % Otherwise issues with Private_Creator
                fnprob = fopen(fid) ;
                warning(['group+1: ',num2str(group+1),' and element+1: ', ...
                    num2str(element+1),' not found in tags for file ',fnprob])
                dicomlookup(group+1,element+1)
            end
            
            VRs = 'LO' ; % suspect unknown Private_Creator
        else
            VRs = values(idx).VR ;
        end
        
        if isequal(VRs,{'US' 'SS'})
            VRs = 'US or SS' ;
        end
        
        if isequal(VRs,{'OW' 'OB'})
            VRs = 'OW/OB' ;
        end
...
switch VRs
    ...
    case 'SQ' % sequence of items
        ...
        if sqlen ~= hex2dec('FFFFFFFF')
            ...
            if ival==0
                val = '' ; % inserted to prevent empty val returned
            end
