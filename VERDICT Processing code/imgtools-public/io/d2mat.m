...
%
% dinfo a structure from DFPARSE (or DATPARSE, DPARSE, DMFPARSE, PARPARSE)
...
%           23 'ORIGINAL\PRIMARY\OTHER' GE?
%           24 'ORIGINAL\PRIMARY\M\ND\NORM') ; % 24
%           25 'ORIGINAL\PRIMARY\PROJECTION IMAGE\M\FFE') ; % 25 MPR Philips
%           26 'ORIGINAL\PRIMARY\ADC_UNSPECIFIED\ADC\UNSPECIFIED') ; % 26 ADC
%           27 'ORIGINAL\SECONDARY\AXIAL' ); % 27 Axial CT
%           28 'ORIGINAL\PRIMARY\M\DIS2D\MFSPLIT') ; % 28 Siemens Mutiframe split(!
%
...
elseif isfield(dinfo, 'EchoNumbers') % FFS 
    ...
    switch outd{ioutd}
       ...
       case 'effTE'
        ...
        case 'echo'
            ...
            elseif isfield(dinfo,'EchoNumbers') 
            ...
            vdims(2+ioutd) = 1;
        case 'EchoTime'
            dp(ioutd).vals = [dinfo.EchoTime] ;
            dp(ioutd).vec = echoTimeVec;
            matp.echoTimeVec = echoTimeVec ;
            vdims(2+ioutd) = 1;
        case 'fa'
...
if isfield(dinfo,'StudyDate')
    usd = unique({dinfo(locs).StudyDate} ) ;
    if length(usd) > 1
        warning('More than one StudyDate in data.')
    end
    matp.StudyDate = usd{1};
end
if isfield(dinfo,'StudyTime')
    ust = unique({dinfo(locs).StudyTime} ) ;
    if length(usd) > 1
        warning('More than one StudyTime in data.')
    end
    matp.StudyTime = ust{1};
end

% Output scaling information
uq_ss = unique([dinfo(locs).Private_2005_100e ] );
uq_rs = unique([dinfo(locs).RescaleSlope] );
uq_ri = unique([dinfo(locs).RescaleIntercept] ) ;

disp('Data used contains ')
disp(" "+length(uq_ri)+" unique RescaleIntercepts in range [" + min(uq_ri) + " "+ max(uq_ri)+']') 
disp(" "+length(uq_rs)+" unique RescaleSlopes in range [" + min(uq_rs) + " " + max(uq_rs)+"]")
disp(" "+length(uq_ss)+" unique ScaleSlopes in range [" + min(uq_ss) + " "+ max(uq_ss)+"]")

switch op
    case 'dv'
        disp('You selected ''dv'' (display value) so only the RescaleSlope and RescaleIntercept are applied.')
    case 'fp'
        disp('You selected ''fp'' (floating point) so all scales are applied.')
    case {'pv', 'pv_single'}
        disp('You selected pixel values so no scaling has been applied.')
end
disp(" ")
