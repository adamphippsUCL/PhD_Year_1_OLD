...
%ESHOW viewing tool for inspecting 2D, 3D, RGB and complex data
% 
% eshow launches a control figure and an image figure. Subsequent calls to
% eshow just add additional image figures. 
%
% Some functionality is provided in the control figure and some from 
% key presses on the active image figure (see below).
%
% Image figure displays ortogonal projections for 3D data.
%
%    FIG = ESHOW launch eshow GUI.
...
%                         'vdim', 'name', 'isrgb', 'geom', 'isRI', 'matp'
%
...
%  S         - plot profile from line ROI on current image as a graph
...
elseif numel(varargin{1}) > 1
          ...
          for ipv = 2:2:nargin
              ...
              switch param
                  ...
                  case 'isRI'
                      isRI = val ;
                  case {'geom', 'matp'}
                      if strcmp(param,'matp')
                          geom = val.geom ;
                          if isfield(val,'StudyDate'), UD.StudyDate = val.StudyDate ; end
                          if isfield(val,'StudyTime'), UD.StudyTime = val.StudyTime ; end
                      else
                          geom = val ;
                      end
                          
                      vdim(1) = geom(1).PixelSpacing_HW(1) ;
                      vdim(2) = geom(1).PixelSpacing_HW(2) ;
                      if isfield(geom(1),'IOP') & length(geom)>1
                          vdim(3) = norm(geom(1).IPP - geom(2).IPP) ;
                      else
                          vdim(3) = geom(1).SliceThickness ;
                      end
                      hasgeom = true ;
                      UD.geom = geom ;
                      noresize = true ;
  ...
  if hasgeom
      set(hcfig.checkbox1,'Value',1)
      set(hcfig.pushbutton7,'Visible',1)
      set(hcfig.pushbutton8,'Visible',1)
      
      oinfo = ori_info(UD.geom(1).IOP, 'geom', UD.geom) ;
      set(hcfig.text29,'String',oinfo.north_str)
      set(hcfig.text30,'String',oinfo.east_str)
      set(hcfig.text32,'String',oinfo.south_str)
      set(hcfig.text33,'String',oinfo.west_str)    
      zstr = oinfo.positive_z_str ;
      zstr(1) = '>' ;
      set(hcfig.text31,'String', zstr)
      if oinfo.isRH == true
          set(hcfig.text31, 'ForegroundColor', [0 0 0])
      else
          set(hcfig.text31, 'ForegroundColor', [1 0 0])
      end
      
  else
      set(hcfig.checkbox1,'Value',0)
      set(hcfig.pushbutton7,'Visible',0)
      set(hcfig.pushbutton8,'Visible',0)
      set(hcfig.text29,'String','')
      set(hcfig.text30,'String','')
      set(hcfig.text32,'String','')
      set(hcfig.text33,'String','')
      set(hcfig.text31,'String','')
...
if UD.hasgeom
    IOP = UD.geom(val).IOP ;
    zpos = dot(UD.geom(val).IPP, cross(IOP(1:3), IOP(4:6))) ;
    set(handles.text28,'String', num2str(zpos) );
end
...
if UD.isrgb
    ...
    if islogical(aw)
      imshow(aw)
        ...
        imshow(aw_full,[double(UD.dspmin) double(UD.dspmax)])
    end
end
...
switch currc
    case '?'
        helptxt = { 'r re-slice', ...
            ...
            'g provide geometry information',...
            'k mIP', ...
            ...
            'S plot profile of current line RI as a graph', ...
        ...
        if isfield(UD,'lroi')
            ...
            if ~ishandle(lroi)
                ...
                if strcmp(currc,'B')
                   ...
                   plot(c)
                   xlabel(['From (',num2str(lroi.Position(1,1)),', ', ...
                       num2str(lroi.Position(1,2)),') to (',...
                       num2str(lroi.Position(2,1)),', ', ...
                       num2str(lroi.Position(2,2)),').'])
                end
            end
        else
    ...
    case 'g'
        if UD.hasgeom
            slc = str2num(get(handles.text22,'String')) ;
            disp(['geom info for slice ', ...
                get(handles.text22,'String'), ' on figure ', ...
                num2str(dfig_number), ' (',dfig_name,')'])
        
            disp([' IPP: [',num2str(UD.geom(slc).IPP(1)), ' ', ...
                num2str(UD.geom(slc).IPP(2)), ' ', ...
                num2str(UD.geom(slc).IPP(3)), ']'])
            
            disp([' IOP(1:3): [',num2str(UD.geom(slc).IOP(1)), ' ', ...
                num2str(UD.geom(slc).IOP(2)), ' ', ...
                num2str(UD.geom(slc).IOP(3)), ']'])
            
            disp([' IOP(4:6): [',num2str(UD.geom(slc).IOP(4)), ' ', ...
                num2str(UD.geom(slc).IOP(5)), ' ', ...
                num2str(UD.geom(slc).IOP(6)), ']'])
            
            CP = [ str2num(get(handles.text20,'String')) , ...
                   str2num(get(handles.text21,'String')) , ...
                   1 ] ;
               
            [coords_lph] = dicom2intrinsic(UD.geom(slc), 'output', 'stackedLPH', ...
                                                    'coords', CP)  ; 
            disp([' Current point LPH: [',num2str(coords_lph(1)), ' ', ...
                num2str(coords_lph(2)), ' ', ...
                num2str(coords_lph(3)), ']'])
        end
    ...
    case char(30) % up arrow
        ...
        if UD.hasgeom
            IOP = UD.geom(UD.coord(Z)).IOP ;
            zpos = dot(UD.geom(UD.coord(Z)).IPP, cross(IOP(1:3), IOP(4:6))) ;
            set(handles.text28,'String', num2str(zpos) );
        end
...
%                     disp(['Temporary force one line montage'])
%                     montage(aw(yl:yu, xl:xu, :, :),'Size',[1 NaN])
                    montage(aw(yl:yu, xl:xu, :, :))
                end
            case 'V'
                if UD.isrgb
...
if UD.hasgeom
    ...
    oinfo = ori_info(UD.geom(1).IOP, 'geom', UD.geom) ;
    set(handles.text29,'String',oinfo.north_str)
    set(handles.text30,'String',oinfo.east_str)
    set(handles.text32,'String',oinfo.south_str)
    set(handles.text33,'String',oinfo.west_str)
    zstr = oinfo.positive_z_str ;
      zstr(1) = '>' ;
      set(handles.text31,'String', zstr)
      if oinfo.isRH == true
...
if ts == 1
    UD.noresize = true ;
    if UD.hasgeom == true 
        set(handles.pushbutton7,'Visible', 1)
        set(handles.pushbutton8,'Visible', 1)
        set(handles.pushbutton9,'Visible', 1)
    else
        set(handles.pushbutton7,'Visible', 0)
        set(handles.pushbutton8,'Visible', 0)
        set(handles.pushbutton9,'Visible', 0)
    end
else
    UD.noresize = false ;
    set(handles.pushbutton7,'Visible', 0)
...
currsl = str2num(get(handles.edit7 ,'String')) ;
% coords_lph = dicom2intrinsic(UD.geom(currsl), 'output','stackedLPH', 'coords', coords) ;
...
hROI = findobj('Tag','eshowROI') ;
for iroi = 1:length(hROI)
    roiUD = hROI(iroi).UserData ;
    
    % Calc  coords_lph for ROI 
    coords = hROI(iroi).Position ; % 2D intrinsic coordinates of polygon 
    coords_lph = dicom2intrinsic(roiUD.geom(roiUD.slice), 'output','stackedLPH', 'coords', coords) ;
    
    currsl = str2num(get(handles.edit7 ,'String')) ;
    coords = dicom2intrinsic(UD.geom(currsl), 'output', 'stackedIntrinsic','coords', coords_lph) ;
    
    roi = images.roi.Polygon(gca,'Position',coords(:,[1 2])) ; % new ROI
    zpD = abs(coords(:,3)-1) ; % Distance from z=1 plane
    if max(zpD) > 1
        roi.StripeColor= [1 0 0] ; % more than 1 slice, hence red warning
    elseif max(zpD) > 0.5
        roi.StripeColor= roi.Color/2 ; % in neighbouring slice, dash line
    end
end
...
messg = 'Select SR ROI for : ' ; 
if isfield(UD,'StudyDate'), messg = [messg,' Date ',UD.StudyDate]; end
if isfield(UD,'StudyTime'), messg = [messg,' Time ',UD.StudyTime]; end
messg = [messg, ' Slice-1: ',num2str(UD.coord(3)-1)];
...
if isfield(UD,'StudyDate')
    if strcmp(UD.StudyDate, dinfo.StudyDate) == 0
        warning('Inconsistent Study Dates')
    end
end
if isfield(UD,'StudyTime')
    if strcmp(UD.StudyTime, dinfo.StudyTime) == 0
        warning('Inconsistent Study Times')
    end
end

figure(fig)
if size(points,1) > 2
    roi = images.roi.Polygon(gca,'Position',points) ;
elseif size(points,1) == 2
    roi = images.roi.Line(gca,'Position',points) ;
else
    warning('Not polygon')
end
    
refframe = str2double(dinfo.ReferencedFrameNumber)+1 ;
if UD.coord(3) ~= refframe 
    warning(['Expected to be on slice: ', num2str(refframe)])
end
