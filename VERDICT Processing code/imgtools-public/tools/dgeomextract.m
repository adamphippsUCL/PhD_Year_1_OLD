...
%
% dinfo is a structure read in using dfparse, datparse, dmfparse, parparse, 
%  dparse or xmlparse
...
geom = struct('IPP',num2cell(zeros([nsl 3]),2), ...
    ...
    'FrameOfReferenceUID', num2cell(repmat('unknown',[nsl 1]),2), ...
    'source',{'Data source'}) ;
...
for isl = 1:nsl
    ...
    if isfield(dinfo,'FrameOfReferenceUID')
        geom(isl).FrameOfReferenceUID = dinfo(loc(isl)).FrameOfReferenceUID ;
    end
